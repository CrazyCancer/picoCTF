...
def get_func(n):
  ...
  func_name = ''
  func_name_offset = n * FUNC_TABLE_ENTRY_SIZE #32
  for i in range(func_name_offset, func_name_offset+FUNC_TABLE_ENTRY_SIZE): #32-64
    if( func_table[i] == ' '):
...
#sau một hồi lần mò thì tôi nhận thấy ở hàm "write_variable()" sẽ làm thay đổi "global func_table", cái mà lưu trữ tên gọi của các hàm 
#ta thấy rằng hàm "ưin()" không có đối số, vậy nên ta cũng cần lựu chọn một hàm không có đối số. Vậy nên ở đây tôi chọn hàm "help_text()"
#bằng cách thực thi "write_variable()" gán "var_name = help_text" và "value = win"
#Giờ đây thay vì gọi đến "help_text()" thì chương trình sẽ gọi đến "win()"  